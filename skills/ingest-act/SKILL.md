...
1. 读取 `docs/wiki/wiki.ingest.json`
2. 从 `pending` 中找到 `id == $ARGUMENTS` 的 target
3. 获取 target 的 `type`、`reason`
4. 读取目标页面的 frontmatter `guidelines` 和 `issues`（如果页面存在）

如果 target 不存在或已在 completed 中 → 报错退出。
...
2. 对比分析：
   ...
   - 无变化 → 跳过
3. 更新页面时同步更新 frontmatter `updated` 为当前秒级 ISO 时间戳
4. 特殊情况：
...
3. 修改时同步更新 frontmatter `updated`

## 更新 wiki.ingest.json

- 从 pending 移除该 target，追加到 completed
- 保存 wiki.ingest.json
