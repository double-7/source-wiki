...
REACT 编排器——对 wiki 知识库进行健康检查。支持全量分析和定向分析。
...
- **无参数**（`$ARGUMENTS` 为空）→ 全量扫描模式，扫描所有 wiki 页面
- **有参数**（如 `auth`）→ 定向扫描模式：
  1. 在 `wiki.json` 的 `modules` 中查找匹配（精确匹配或包含关键词）
  2. 匹配到 → 确定扫描范围：该模块的 module 页面 + 其 features 页面 + 涉及该模块的 flows 页面
  3. 未匹配到 → 报错："未找到模块 '{参数}'。可用模块：{列出 modules 的 key}"，退出

### 3. 定向模式范围外 issues 交互（仅定向模式）

扫描范围内的页面 frontmatter issues 数量为 N（范围内），范围外页面 issues 数量为 K。仅当 K > 0 时触发交互：

使用 AskUserQuestion 询问：
```
在 {模块名} 范围内发现 {N} 个待处理 issues。
其他模块中有 {K} 个待处理 issues。
是否同时处理？
```

选项：
- "只处理 {模块名} 范围" → 仅扫描指定模块的页面
- "全面修复" → 扩大扫描到所有模块
...
`scope` 始终写入：定向模式为模块名（如 `"auth"`），全量模式为空字符串 `""`。
...
将 findings 分为三组：
- **safe（已修复）**：lint-act 已直接执行（含 issues 直接修复），报告中标注"已修复"
- **content（待确认）**：需要用户确认的内容修复方案（含 content 级别 issues）
- **none（仅报告）**：无法自动修复的问题

### 2.5. issues 处理摘要

输出本次 lint 处理的 issues 汇总：
