...
- **初始化（init）**：纯编排器。Plan → Fill → Refine 三阶段通过 fork agent 委派执行。自动检测 wiki.json 状态从断点继续。在用户会话中运行，负责状态管理和用户交互（检查点确认）。
- **规划（plan）**：在 fork 模式下自主运行。由 init 通过 Skill tool 委派，扫描源码元数据、制定模块划分方案、生成 wiki 骨架。在自己的上下文窗口中运行，不受 init 上下文容量限制。
- **填充（fill）**：在 fork 模式下自主运行。由 init 通过 Skill tool 逐模块委派，自包含执行（读 wiki.json → 分析源码 → 创建页面 → 更新 wiki.json）。
- **精炼（refine）**：在 fork 模式下自主运行。由 init 通过 Skill tool 委派，自包含执行跨模块一致性检查、flow 页面创建、overview 完善。
- **增量摄取（ingest）**：在 fork 模式下自主运行。用于源码变更后增量同步 wiki。
- **查询（query）**：基于 wiki 回答问题。沉淀必须由用户明确触发，agent 仅输出建议。
...
**wiki.json 更新规则**：任何修改了 wiki 内容的操作完成后，都必须更新 `wiki.json`——至少 bump `revision`。如果页面被创建或删除，还须同步更新对应的 `features`/`modules`/`flows` 条目。这确保增量 ingest 的 git anchor 和 features.source 映射始终与实际 wiki 状态一致。
...
用途：
- init 各阶段的状态机和断点恢复（process.phase、process.pendingModules）
...
```json
{
  ...
  "process": {
    "phase": "filling",
    "pendingModules": ["api-channel", "http-management"],
    "completedModules": ["protocol", "sdk", "udp-comm"],
    "processingOrder": ["protocol", "sdk", "udp-comm", "logging"],
    "estimatedFlows": ["file-sync-flow", "app-startup-flow"]
  },

  "architectures": {
    "techStack": ["Java 11", "Netty 4.1.124"],
    "pages": ["docs/wiki/architectures/overview.md"]
  },

  "modules": {
    "protocol": {
      "source": "jchannel-lib/src/main/java/team/vi/jchannel/nlib/",
      "features": ["udp-protocol-definition", "tcp-api-protocol"],
      "dependencies": [],
      "exports": [],
      "conventions": [],
      "page": "docs/wiki/modules/protocol.md"
    }
  },
  ...
  "features": {
    "tcp-api-protocol": {
      "module": "protocol",
      "source": ["nlib/JChannelProtocol.java", "nlib/api/ConnectClient.java"],
      "imports": [],
      "page": "docs/wiki/features/tcp-api-protocol.md"
    }
  },

  "flows": {
    "file-sync-flow": {
      "modules": ["file-channel", "udp-comm", "protocol"],
      "page": "docs/wiki/flows/file-sync-flow.md"
    }
  }
}
```
...
| `process.phase` | string | 当前阶段：`planned` / `filling` / `filled` / `refining` / `completed` |
| `process.pendingModules` | string[] | 待 Fill 处理的模块列表（process.phase != completed 时存在） |
| `process.completedModules` | string[] | 已完成 Fill 的模块列表（process.phase != completed 时存在） |
| `process.processingOrder` | string[] | 模块处理顺序，按依赖关系从底层到上层（process.phase != completed 时存在） |
| `process.estimatedFlows` | string[] | 预估的跨模块流程（process.phase != completed 时存在） |
...
- **增量 ingest 完成**：更新受影响 features 的 `source`/`imports`，`revision` +1，更新 `lastUpdated`
- **query 产生沉淀**：`revision` +1（仅当用户明确要求沉淀时），新 feature 沉淀时同时写入 `features` 条目
- **lint 修复 wiki**：`revision` +1，如涉及页面增删则同步更新 `features`/`modules` 条目
